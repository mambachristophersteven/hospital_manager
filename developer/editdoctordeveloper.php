...
$id = $_GET['editid'];
$sqlview= "SELECT * from `doctors` WHERE id=$id";
...
$admin_password=$rowview['password'];
...
if(isset($_POST['submit'])){
    ...
    if(empty($firstname)){
        $error="Enter the firstname of the admin";
    }
    else{
        if(empty($lastname)){
            $error="Enter the lastname of the admin";
        }
        else{
            if(empty($age)){
                $error="Enter the age of the admin";
            }
            else{
                if(empty($marital_status)){
                    $error="Select the marital status of the admin";
                }
                else{
                    if(empty($date_of_birth)){
                        $error="Select the date of birth of the admin";
                    }
                    else{
                        if(empty($email)){
                            $error="Enter the email address of the admin";
                        }
                        else{
                            if(empty($address)){
                                $error="Enter the residential address of the admin";
                            }
                            else{
                                if(empty($landmark)){
                                    ...
                                    if(empty($hometown)){
                                        $error="Enter the hometown of the admin";
                                    }
                                    else{
                                        if(empty($telephone1)){
                                            $error="Enter the contact number of the admin";
                                        }
                                        else{
                                            if(empty($religion)){
                                                $error="Enter the religion of the admin";
                                            }
                                            else{
                                                if(empty($religion)){
                                                    $error="Select the religion of the admin";
                                                }
                                                else{
                                                    if(empty($religion)){
                                                        $error="Enter the religion of the admin";
                                                        ...
                                                        if(empty($religion)){
                                                            $error="Enter religion of the admin";
                                                        }
                                                        else{
                                                            if(empty($religion)){
                                                                $error="Specify the religion of the admin";
                                                            }
                                                            else{
                                                                if($imagefilesize===0){
                                                                    $error="Reselect the image of the admin";
                                                                }
                                                                else{
                                                                    ...
                                                                    if(in_array($imagefilenameextension,$extensions)){
                                                                        ...
                                                                        $sqlinsert= "UPDATE `doctors` SET id ='$id',firstname='$firstname',lastname='$lastname',age='$age',gender='$gender',marital_status='$marital_status',image='$upload_image',date_of_birth='$date_of_birth',address='$address',gps_address='$gps_address',landmark='$landmark',hometown='$hometown',
                                                                        othername='$othername',department='$department',telephone1='$telephone1',telephone2='$telephone2',religion='$religion',spouse_firstname='$spouse_firstname',spouse_lastname='$spouse_lastname',spouse_telephone='$spouse_telephone',date_added='$currentDate' WHERE id ='$id'";
...
<head>
    ...
    <title>Redeemer Academy - Edit Doctor Profile</title>
                ...
                <div class="form">
                    <form action="" method="post" enctype="multipart/form-data" id="form">
                        ...
                        <div class="form-divisions">
                            ...
                            <input type="text" name="spouse_name" id="spouse_firstname" placeholder="if married, enter spouse firstname" value=<?php echo $spouse_firstname;?>>
                        </div>
                        <div class="form-divisions">
                            <h4 class="select-heading">spouse lastname</h4>
                            <input type="text" name="spouse_name" id="spouse_lastname" placeholder="if married, enter spouse lastname" value=<?php echo $spouse_lastname;?>>
                        </div>
                        <div class="form-divisions">
                        ...
                        <div class="form-divisions">
                            ...
                            <input type="text" name="department" id="department" placeholder="enter department" maxlength="10" value=<?php echo $department;?>>
                        </div>
                    ...
                    <div class="form-divisions">
                        <button id="cancel-add" onclick="location.href='./doctorsdeveloper.php'">Cancel</button>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function checkInputs(){
            ...
            if(imageInput.value===''){
                ...
                errorMessage.innerText = "Reselect a picture of the doctor";
            }
            else{
                imageContainer.style.border = "3px solid #7380ec";
            }

            if(firstnameValue !='' && lastnameValue !='' && ageValue !='' && emailValue !='' && addressValue !='' && genderValue !='' && landmarkValue !='' && hometownValue !='' && religionValue !='' && telephone1Value !='' && imageInput.value !='' && dateOfEmploymentValue !=''  && dateOfBirthValue !='' && departmentValue !=''){
